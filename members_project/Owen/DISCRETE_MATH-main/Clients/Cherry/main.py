...
def main():
    ...
    while True:
        ...
        print("🧠 Cherry Albia’s Algorithm Arena: Booting Up Cognitive Core… 🧠\n")
        print(random.choice([
            "What field of logic do you wish to master today?",
            "Choose your next intellectual expedition:",
            "Step into a realm of structured problem-solving:",
            "Select your algorithmic journey:",
        ]))

        for idx, module in enumerate(modules, start=1):
            print(f" {idx}. ⚙️ {module.__name__}")
        print(" 0. 🛑 Leave the Lab")

        try:
            choice = int(input("\n📊 Enter your selection (0 to exit): "))
        except ValueError:
            print(random.choice([
                "\n🚫 That’s not a number. Please try again.",
                "\n❌ Numbers only! Let’s give it another go.",
                "\n⚠️ Invalid input — please enter a digit.",
            ]))
            sleep(1.5)
            continue
        ...
        if choice == 0:
            print(random.choice([
                "\n👋 Thanks for decoding with me! Until next time!",
                "\n🔚 Exiting the lab. Stay curious!",
                "\n💫 That was enlightening! Come back soon.",
            ]))
            break
        ...
        if 1 <= choice <= len(modules):
            ...
            while True:
                clear_screen()
                print(f"🔬 Launching {selected_module.__name__} Module...\n")
                selected_module()

                again = input(random.choice([
                    "\n🔁 Would you like to explore this module again? (y/N): ",
                    "\n♻️ Rerun this module? (y/N): ",
                    "\n🔄 Give this another try? (y/N): ",
                ])).strip().lower()
            ...
            print(random.choice([
                "\n⚠️ That choice isn’t on the list.",
                "\n🚫 Invalid option. Pick from the menu.",
                "\n❗ Please choose a valid number shown above.",
            ]))
            sleep(1.5)
