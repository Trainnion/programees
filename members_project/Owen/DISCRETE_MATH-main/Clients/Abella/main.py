...
def main():
    ...
    while True:
        ...
        print("🧠 Jonh Vincent Abella’s Algorithm Command Hub: Powering Up Systems… 🧠\n")
        print(random.choice([
            "Which logic portal would you like to unlock?",
            "Pick your next algorithmic mission:",
            "Engage with a module from the vault below:",
            "Select a domain to explore and conquer:",
        ]))
        ...
        try:
            choice = int(input("\n📊 Choose a module (0 to exit): "))
        except ValueError:
            print(random.choice([
                "\n🚫 That’s not a valid number. Try again.",
                "\n❌ Numbers only. Let's correct that.",
                "\n⚠️ Error: Digits expected. Please retry.",
            ]))
            sleep(1.5)
            continue
        ...
        if choice == 0:
            print(random.choice([
                "\n👋 Session complete. Until our next algorithmic journey!",
                "\n🔚 Logging out. Stay curious and sharp!",
                "\n🌌 Thanks for navigating the dataverse!",
            ]))
            break
        ...
        if 1 <= choice <= len(modules):
            ...
            while True:
                clear_screen()
                print(f"🔬 Executing {selected_module.__name__} Module...\n")
                selected_module()

                again = input(random.choice([
                    "\n🔁 Replay this module? (y/N): ",
                    "\n♻️ Run this logic again? (y/N): ",
                    "\n🔄 Try another pass? (y/N): ",
                ])).strip().lower()
            ...
            print(random.choice([
                "\n⚠️ Invalid number. That’s not on the list.",
                "\n🚫 Option not recognized. Choose again.",
                "\n❗ That’s outside the selection range. Retry.",
            ]))
            sleep(1.5)
